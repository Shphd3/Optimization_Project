...
def forward(x_t, beta, Q0, Alpha, L0, C0_all, Wq, We, Rsrp, Conn, Capa, ALL_Users_Traffic, ALL_Cells_Bw):
    ...
    # 1. 计算流量分配概率矩阵
    Prob = scheduling_prob(x_t, Rsrp, Conn, beta)

    # 2. 计算小区负载和带宽利用率
    # Lambda_mn = A_n * Prob_mn
    # L_m = sum_n Lambda_mn / Capa_mn
    L_m = np.sum((ALL_Users_Traffic[np.newaxis, :] * Prob) / (Capa + 1e-9), axis=1)
    Load_m = L_m / ALL_Cells_Bw

    # 3. 计算目标函数值
    # Part I: QoS下降成本
    QoS_Cost_All = func_Q(Load_m, ALL_Cells_Bw, Q0, L0, Alpha)
    obj_Q = Wq * np.sum(QoS_Cost_All)
    # Part II: Energy成本
    Energy_Cost_All = func_E(Load_m, ALL_Cells_Bw, C0_all)
    obj_E = We * np.sum(Energy_Cost_All)

    obj = obj_Q + obj_E

    # 计算单位成本 (Unit Cost / Marginal Cost)
    dQ_dl = grad_Q_to_l(Load_m, ALL_Cells_Bw, Q0, L0, Alpha)
    dE_dl = grad_E_to_l(Load_m, ALL_Cells_Bw, C0_all)
    Unit_Cost = Wq * dQ_dl + We * dE_dl

    # 缓存中间变量，供 Backward 计算梯度使用
    cache['Prob'] = Prob
    cache['Load_m'] = Load_m
    cache['L_m'] = L_m
    cache['obj_Q'] = obj_Q
    cache['obj_E'] = obj_E
    cache['Unit_Cost'] = Unit_Cost
    cache['Unit_Cost_Q'] = dQ_dl
    cache['Unit_Cost_E'] = dE_dl
...
def backward(x_t, cache, M, N, beta, Q0, Alpha, L0, C0_all, Wq, We, Conn, Capa, ALL_Users_Traffic, ALL_Cells_Bw):
    ...
    dQ_dl = grad_Q_to_l(Load_m, ALL_Cells_Bw, Q0, L0, Alpha)
    dE_dl = grad_E_to_l(Load_m, ALL_Cells_Bw, C0_all)
    dU_dl = Wq * dQ_dl + We * dE_dl

    # 根据链式法则计算梯度 grad_obj_to_x
    # U_mn = (dU/dl_m) * (A_n / Capa_mn)
    U_mn = dU_dl[:, np.newaxis] * (ALL_Users_Traffic[np.newaxis, :] / (Capa + 1e-9))
    # U_bar_n = sum_m Prob_mn * U_mn
    U_bar_n = np.sum(Prob * U_mn, axis=0)
    # grad_m = beta * sum_n Prob_mn * (U_mn - U_bar_n)
    grad_obj_to_x = beta * np.sum(Prob * (U_mn - U_bar_n[np.newaxis, :]), axis=1)
